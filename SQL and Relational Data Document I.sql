...
-- Query for all first_name data that starts with a K and ends with th lowercase
...
select MIN(amount) as Minimum_Payments
from payment 
where amount > 7.99;

-- Query to display MAX amount greater than 7.99
select MAX(amount) as Maximum_Payments
from payment 
where amount > 7.99;
...
-- Query to display customer_ids with the SUM amount for each customer_id
select customer_id, SUM(amount)
from payment
group by customer_id
order by customer_id desc;

-- Query to display customer_ids with the amount for each customer_id
...
-- Query to display customer_ids that show up more than 0 times
